...
1. メニュー「バックログ」→ **🆕 バックログ / エピックを追加** で、プロダクト名を指定して `バックログ_` / `エピック_` の対を展開する（同名シートがあればデータは保持）。
2. `バックログ_` シートに直接入力する（内容が入った時点で ID が自動採番される）。Epic は対になる `エピック_` シートで管理する。
3. 名前を変えて同じ操作を繰り返すと、複数のプロダクトを管理できる。
...
2. メニュー「バックログ」→ **🆕 バックログ / エピックを追加（名前を指定）** を実行する（プロダクト名を聞かれるので入力する。初回は権限の承認が必要）
3. `バックログ_{名前}` と `エピック_{名前}`（+ 🔢 ID管理）ができ、ID 採番・プルダウンが有効になる
4. メニューをもう一度実行し、別のプロダクト名を入れると**同じブックに複数のプロダクト**を追加できる
...
| [`epic-sheets.gs`](epic-sheets.gs) | Epic シートのヘッダー・列幅・初期サンプル行の定義。 |
| [`ids.gs`](ids.gs) | 🔢 ID管理 シートの読み書き、シートごとの ID 採番、`onEdit` による自動採番。 |
| [`menu.gs`](menu.gs) | カスタムメニュー（`onOpen`）。シート追加・ID再同期の入口。 |
| [`validation.gs`](validation.gs) | Epic・ステータス・着手可能性・ポイント の入力規則。 |
